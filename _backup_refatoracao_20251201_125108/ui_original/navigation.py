...
class NavigationManager:
    ...
    def navigate_to(self, module_name: str, callback: Optional[Callable] = None):
        ...
        try:
            ...
            registrar_log("Navegação", f"Navegação de '{old_module}' para '{module_name}'", "INFO")
            ...
            registrar_log("Navegação", f"Erro na navegação para '{module_name}': {e}", "ERROR")
    ...
    def _update_ui_for_module(self, module_name: str):
        ...
        module_configs = {
            'main': {
                'title': "IntegraGAL - Menu Principal",
                'actions': []
            },
            'extracao': {
                'title': "IntegraGAL - Mapeamento da Placa",
                'actions': []
            },
            'analise': {
                'title': "IntegraGAL - Análise",
                'actions': []
            },
            'resultados': {
                'title': "IntegraGAL - Resultados",
                'actions': []
            },
            'gal': {
                'title': "IntegraGAL - Envio GAL",
                'actions': []
            },
            'admin': {
                'title': "IntegraGAL - Administração",
                'actions': []
            },
            'usuarios': {
                'title': "IntegraGAL - Gerenciar Usuários",
                'actions': []
            }
        }
        ...
        for action in config['actions']:
            try:
                ...
                registrar_log("Navegação", f"Erro ao executar ação do módulo '{module_name}': {e}", "ERROR")
    ...
    def get_module_info(self, module_name: str) -> dict:
        ...
        info = {
            ...
            'navigation_count': len([h for h in self.navigation_history if h['to'] == module_name])
        }
