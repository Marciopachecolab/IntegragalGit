...
6. Verificar que mudanças persistem
...
def test_1_end_to_end_create_novo_exame():
    ...
    print("[PASSO 2] Preenchendo formulário com dados válidos...", end=" ")
    ...
    print("[PASSO 3] Coletando dados do formulário...", end=" ")
    ...
    assert found, f"Exame '{cfg.nome_exame}' não aparece na lista"
...
def test_2_end_to_end_editar_exame(exam_slug):
    ...
    # 3. Verificar que campos estão preenchidos

    print("[PASSO 3] Verificando que campos estão pré-preenchidos...", end=" ")
    ...
    dialog.text_comentarios.insert("1.0", "Comentário atualizado")
    ...
    assert updated_cfg.comentarios == "Comentário atualizado"
    ...
    print("[PASSO 7] Salvando alterações em JSON...", end=" ")
...
def test_3_end_to_end_deletar_exame(exam_slug):
    ...
    print("[PASSO 5] Verificando remoção da lista...", end=" ")

    exames = editor.load_all_exams()

    found = any(nome == cfg.nome_exame for nome, _ in exames)

    # Nota: pode ainda estar no CSV, então não falha se encontrado
...
def main():
    ...
    for test_name, result in results:
        ...
        status = "âœ" PASSOU" if result is True else ("âœ— FALHOU" if result is False else "⊂˜ PULADO")
