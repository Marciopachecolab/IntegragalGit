...
from ..utils.db_utils import salvar_historico_processamento
...
CSV_COLUMNS_MODEL = [
    ...
    "coronavirusnl63", "coronavirushku1", "coronavirusoc43",
    "adenovirus", "vsincicialresp", "metapneumovirus", "rinovirus",
    "bocavirus", "enterovirus", "parainflu_1", "parainflu_2", "parainflu_3", "parainflu_4",
    "coronavirus", "influenzaahn", "metapneumovirua", "metapneumovirub",
    "mycoplasma", "parechovÃ­rus", "vsincicialrespa", "vsincicialrespb",
    "influenzaah_3", "influenzaah_1", "influenzaah_7"
]
...
def _convert_ct_value(value: Any) -> Optional[float]:
    ...
    try:
        ...
        registrar_log("ConversÃ£o", f"Falha ao converter valor de CT: '{value}'", level='WARNING')
        return np.nan

def _generate_well_pairs() -> List[Tuple[str, str]]:
    """Gera pares de poÃ§os (ex: A1+A2, A3+A4, ..., H11+H12) para anÃ¡lise."""
    return [(f"{row}{col}", f"{row}{col + 1}") for row in "ABCDEFGH" for col in range(1, 13, 2)]

def load_and_preprocess_data(filepath: str) -> pd.DataFrame:
    """Carrega e prÃ©-processa dados da placa com tratamento robusto."""
    registrar_log("AnÃ¡lise Dados", f"Carregando: {os.path.basename(filepath)}", level='INFO')
        ...
        try:
            if ext in ['.xls', '.xlsx', '.xlsm']:
                ...
                for sep in separators:
                    try:
                        df = pd.read_csv(filepath, sep=sep, skiprows=skip, on_bad_lines='skip')
            ...
            if df is not None and not df.empty:
                ...
                if any('Well' in col or 'PoÃ§o' in col or 'Poco' in col for col in df.columns):
                    registrar_log("Leitura", f"Arquivo lido, pulando {skip} linhas", level='INFO')
    ...
    if df is None:
        raise ValueError(f"NÃ£o foi possÃ­vel identificar o cabeÃ§alho em {os.path.basename(filepath)}")
    
    # Mapeamento de nomes de colunas para padronizaÃ§Ã£o
    column_mapping = {
        'CÑ‚': 'CT', 'Cq': 'CT', 'Cycle Threshold': 'CT', 'Ct': 'CT', 'Threshold Cycle': 'CT',
        'Cicles': 'CT', 'Target Name': 'Target', 'Alvo': 'Target', 'Nome do Alvo': 'Target',
        'Marker': 'Target', 'Sample Name': 'Sample', 'Amostra': 'Sample', 'IdentificaÃ§Ã£o': 'Sample',
        'ID': 'Sample', 'Well Position': 'Well', 'PoÃ§o': 'Well', 'Poco': 'Well', 'PosiÃ§Ã£o': 'Well',
        'Location': 'Well'
    }
...
class PlateAnalysisProcessor:
    ...
    def __init__(self, df_raw_data: pd.DataFrame, filename: str, extraction_data: Optional[dict] = None):
        ...
        registrar_log("PlateAnalysisProcessor", f"Processador inicializado para arquivo: {os.path.basename(filename)}", level='INFO')
    ...
    def _extract_plate_metadata(self) -> None:
        ...
        if data_placa != 'Data Desconhecida':
            try:
                data_formatada = datetime.strptime(data_placa, '%Y%m%d').strftime('%d/%m/%Y')
            except ValueError:
                registrar_log("PlateAnalysisProcessor", f"Formato de data invÃ¡lido no nome do arquivo: {data_placa}", level='WARNING')
        ...
        registrar_log("PlateAnalysisProcessor", f"Metadados da placa extraÃ­dos: {num_placa}, {data_formatada}", level='INFO')
    ...
    def _analyze_well_pair(self, well1: str, well2: str) -> dict:
        ...
        rp_cts = rp_subset['CT'].dropna() if 'CT' in rp_subset.columns else pd.Series(dtype=float)
        rp_valido = any(CT_RP_MIN <= ct <= CT_RP_MAX for ct in rp_cts)

        # Determina se Ã© um controle
        is_control = any(id in sample_upper for id in CONTROL_NEGATIVE_IDENTIFIERS + CONTROL_POSITIVE_IDENTIFIERS)
        ...
        for alvo in TARGET_LIST:
            ...
            ct_alvo = alvo_subset['CT'].dropna() if 'CT' in alvo_subset.columns else pd.Series(dtype=float)
        ...
        return {
            ...
            "RP (CT)": ', '.join([f"{ct:.2f}".replace('.', ',') for ct in rp_cts]) if not rp_cts.empty else 'â€”',
            "ValidaÃ§Ã£o": status_validacao,
            "Alvos Detectados": ', '.join(alvos_detectados) or "Nenhum Alvo DetectÃ¡vel",
    ...
    def analyze_plate(self) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        registrar_log("PlateAnalysisProcessor", "Iniciando anÃ¡lise da placa.", level='INFO')
        ...
        for well1, well2 in well_pairs:
            ...
            if not self.df_raw_data['Well'].isin([well1, well2]).any():
                registrar_log("PlateAnalysisProcessor", f"PoÃ§os {well1}+{well2} nÃ£o encontrados nos dados brutos. Pulando.", level='DEBUG')
                continue
            ...
            if not sample_name:
                registrar_log("PlateAnalysisProcessor", f"Amostra nÃ£o encontrada para o poÃ§o {well1}. Pulando.", level='DEBUG')
                continue
        ...
        registrar_log("PlateAnalysisProcessor", f"AnÃ¡lise da placa concluÃ­da. {len(resultados)} pares de poÃ§os processados.", level='INFO')
    ...
    for _, row in df_validos.iterrows():
        for internal_col, csv_col in TARGET_RESULT_TO_CSV_COLUMN_MAPPING.items():
            if row.get(internal_col, "") == "DetectÃ¡vel":
                counts[csv_col] += 1
    registrar_log("RelatÃ³rio AnÃ¡lise", "Contagem de amostras detectÃ¡veis realizada.", level='INFO')
    return counts

def display_analysis_report(analysis_info: Dict[str, Any], detectable_counts: Dict[str, int]) -> None:
    ...
    for csv_col in sorted(detectable_counts.keys(), key=lambda c: HUMAN_READABLE_TARGET_NAMES.get(c, c)):
    ...
    registrar_log("RelatÃ³rio AnÃ¡lise", "RelatÃ³rio de anÃ¡lise exibido ao usuÃ¡rio.", level='INFO')
...
def generate_detectable_plot(detectable_counts: Dict[str, int]) -> None:
    ...
    if not detectable_counts or all(v == 0 for v in detectable_counts.values()):
        messagebox.showinfo("GrÃ¡fico de DetecÃ§Ã£o", "Nenhum alvo detectÃ¡vel para gerar o grÃ¡fico.")
        registrar_log("GrÃ¡fico DetecÃ§Ã£o", "Nenhum alvo detectÃ¡vel para gerar o grÃ¡fico.", level='INFO')
        return

    labels = [HUMAN_READABLE_TARGET_NAMES.get(k, k) for k, v in detectable_counts.items() if v > 0]
    ...
    plt.tight_layout()
    plt.show()
    registrar_log("GrÃ¡fico DetecÃ§Ã£o", "GrÃ¡fico de amostras detectÃ¡veis gerado e exibido.", level='INFO')
...
def generate_plate_map(df_processed_results: pd.DataFrame) -> None:
    ...
    for c in cols:
        ax.text(c - 0.5, -0.7, str(c), ha='center', va='center', fontsize=10, weight='bold')
    ...
    for _, row in df_processed_results.iterrows():
        ...
        rect = Rectangle((col_idx, row_idx), 2, 1,
                         facecolor=facecolor, edgecolor='black', linewidth=0.8, alpha=0.8)
        ax.add_patch(rect)
        
        # Adiciona textos dentro do poÃ§o
        texts = []
        texts.append(("âœ“" if row.get("Selecionado") == True else "[ ]") + f" {row['Sample']}")
    ...
    plt.show()
    plt.close('all')
    registrar_log("GrÃ¡fico DetecÃ§Ã£o", "Mapa da placa gerado e exibido.", level='INFO')
...
def export_analysis_to_csv(df_processed_results: pd.DataFrame, lote_kit: str) -> None:
    ...
    registrar_log("ExportaÃ§Ã£o CSV", "Iniciando exportaÃ§Ã£o de resultados para CSV (funÃ§Ã£o interna).", level='INFO')
    lines_to_export = []
    ...
    detectable_counts = {col_csv: 0 for col_csv in TARGET_RESULT_TO_CSV_COLUMN_MAPPING.values()}

    df_selecionados = df_processed_results[df_processed_results["Selecionado"] == True]
    if df_selecionados.empty:
        messagebox.showwarning("Nenhuma Amostra", "Nenhuma amostra vÃ¡lida para exportaÃ§Ã£o.")
        registrar_log("ExportaÃ§Ã£o CSV", "Nenhuma amostra vÃ¡lida para exportaÃ§Ã£o.", level='WARNING')
        return
    ...
    try:
        ...
        registrar_log("ExportaÃ§Ã£o CSV", f"Erro ao preparar dados para exportaÃ§Ã£o ou escrever log: {e}", level='ERROR')
    ...
    if output_filepath:
        try:
            ...
            registrar_log("ExportaÃ§Ã£o CSV", f"Arquivo salvo em: {output_filepath}", level='INFO')

            msg = f"Arquivo salvo em: {output_filepath}\nLog salvo em: {log_file_path}\n\nContagem de DetectÃ¡veis:\n"
            for col_csv in sorted(detectable_counts.keys(), key=lambda c: HUMAN_READABLE_TARGET_NAMES.get(c, c)):
            ...
            registrar_log("Erro ExportaÃ§Ã£o", f"Falha ao salvar CSV: {e}", level='ERROR')
            messagebox.showerror("Erro", f"Falha ao salvar CSV: {e}")
    else:
        messagebox.showwarning("Cancelado", "ExportaÃ§Ã£o cancelada pelo usuÃ¡rio.")
        registrar_log("ExportaÃ§Ã£o CSV", "ExportaÃ§Ã£o cancelada pelo usuÃ¡rio.", level='INFO')
...
class AnalysisWindow(AfterManagerMixin, ctk.CTkToplevel):
    ...
    def __init__(self, master, dados_extracao=None):
        ...
        registrar_log("AnalysisWindow", "Janela de anÃ¡lise inicializada.", level='INFO')
        self._init_analysis()
    ...
    def _init_analysis(self):
        ...
        if not file_path:
            registrar_log("AnÃ¡lise Cancelada", "Nenhum arquivo de dados da placa selecionado.", level='INFO')
            self._safe_close()
            return
        ...
        try:
            ...
            processor = PlateAnalysisProcessor(df_raw, file_name, extraction_data=self.dados_extracao)
            ...
            registrar_log("AnalysisWindow", "AnÃ¡lise concluÃ­da e histÃ³rico salvo.", level='INFO')

        except Exception as e:
            registrar_log("Erro AnÃ¡lise", f"Erro durante a anÃ¡lise da placa: {e}", level='ERROR')
    ...
    def _safe_close(self):
        ...
        try:
            ...
            if self.winfo_exists():
                self.destroy()
            registrar_log("Fechamento", "Janela de anÃ¡lise fechada com seguranÃ§a", level='INFO')
        except Exception as e:
            registrar_log("Erro Fechamento", f"Erro crÃ­tico ao fechar janela: {e}", level='ERROR')
            
def abrir_analise(master, dados_extracao=None):
    """FunÃ§Ã£o exposta para criar a janela de anÃ¡lise."""
    registrar_log("AnÃ¡lise Integrada", "Chamada para abrir janela de anÃ¡lise.", level='INFO')
    return AnalysisWindow(master=master, dados_extracao=dados_extracao)

def analisar_placa_vr1e2_7500(master_app_root: ctk.CTk, dados_extracao: dict, parte_placa: str) -> pd.DataFrame:
    """FunÃ§Ã£o de anÃ¡lise para VR1e2 Biomanguinhos 7500, integrada ao sistema principal."""
    try:
        registrar_log("AnÃ¡lise Integrada", "Iniciando anÃ¡lise com dados de extraÃ§Ã£o", level='INFO')
        
        janela_analise = abrir_analise(master=master_app_root, dados_extracao=dados_extracao)
        janela_analise.grab_set()
        master_app_root.wait_window(janela_analise)
        
        if janela_analise.df_results is not None and not janela_analise.df_results.empty:
            registrar_log("AnÃ¡lise Integrada", "AnÃ¡lise concluÃ­da com sucesso", level='INFO')
            return janela_analise.df_results
        else:
            registrar_log("AnÃ¡lise Integrada", "AnÃ¡lise retornou DataFrame vazio ou None", level='WARNING')
            return pd.DataFrame()
    except Exception as e:
        registrar_log("Erro Interface", f"Falha na interface de anÃ¡lise: {str(e)}", level='ERROR')
...
if __name__ == "__main__":
    ...
    os.chdir(BASE_DIR)
    registrar_log("ExecuÃ§Ã£o AutÃ´noma", "Iniciando execuÃ§Ã£o autÃ´noma do mÃ³dulo de anÃ¡lise.", level='INFO')
    ...
    if root_app.winfo_exists():
        root_app.destroy()
    registrar_log("ExecuÃ§Ã£o AutÃ´noma", "ExecuÃ§Ã£o autÃ´noma do mÃ³dulo de anÃ¡lise finalizada.", level='INFO')
