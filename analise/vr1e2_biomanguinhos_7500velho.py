...
ALL_TARGETS = [
    target.upper() for target in TARGET_LIST + ["RP"]
...
def _processar_ct(ct_value: Any) -> Optional[float]:
    ...
    if isinstance(ct_value, str):
        ...
        if ct_value in ("UNDETERMINED", "NA", "", "N/D"):
            return None
        try:
            return float(ct_value.replace(",", "."))
...
def _interpretar_resultado(
    ct_rp1: Optional[float], ct_rp2: Optional[float], ct_alvo: Optional[float]
) -> str:
    """
...
def preview_extracao_sheet(caminho_arquivo: str) -> None:
    ...
    try:
        ...
        df_preview = pd.read_excel(
            caminho_arquivo, usecols="B:M", skiprows=9, nrows=8, header=None
        )
        ...
        label = ctk.CTkLabel(
            preview_window,
            text="Matriz da Planilha (B10:M17):",
        ...
        registrar_log(
            "Pré-Visualização",
            "Pré-visualização da matriz B10:M17 exibida com sucesso.",
            "INFO",
        )
        preview_window.wait_window()
    except Exception as e:
        registrar_log(
            "Pré-Visualização", f"Erro ao exibir pré-visualização: {e}", "ERROR"
        )
        messagebox.showerror(
            "Erro na Pré-Visualização", f"Não foi possível carregar a matriz. Erro: {e}"
        )


def analisar_placa_vr1e2_7500(
    caminho_arquivo_resultados: str, dados_extracao_df: pd.DataFrame, parte_placa: int
    ...
    try:
        ...
        if len(df_raw.columns) != len(expected_columns):
            raise ValueError(
                f"Número de colunas incorreto: esperado {len(expected_columns)}, encontrado {len(df_raw.columns)}."
            )
        ...
        registrar_log(
            "Análise VR1e2", f"DataFrame após leitura: {df_raw.shape} linhas", "DEBUG"
        )
    ...
    registrar_log(
        "Análise VR1e2", f"DataFrame filtrado: {df_filtered.shape} linhas", "DEBUG"
    )
    ...
    if df_proc["CT"].isna().all():
        registrar_log(
            "Análise VR1e2", "Aviso: Todos os valores de CT são NaN.", "WARNING"
        )
    ...
    if "Poço" not in dados_extracao_df.columns:
        raise KeyError(
            "dados_extracao_df deve conter a coluna 'Poço' com a designação dos poços (ex: A1)."
        )
    ...
    # Linha comentada devido a alerta do ruff (F841): variável 'merged' atribuída mas não utilizada diretamente.
    # Linha comentada devido a alerta do ruff (F841): variável 'merged' atribuída mas não utilizada diretamente.
    # merged = pd.merge(
    #     dados_extracao_df, df_proc, left_on="Poço", right_on="WELL", how="left"
    # )
    ...
    # Linha comentada devido a alerta do ruff (F841): variável 'targets_poço2' atribuída mas não utilizada diretamente.
    ...
    for samp in samples:
        ...
        pocos = (
            dados_extracao_df.loc[
                dados_extracao_df["Amostra"].astype(str) == str(samp), "Poço"
            ]
            .astype(str)
            .str.strip()
            .tolist()
        ...
        def _ct_for(target, well):
            if well is None:
                return None
            s = df_proc[
                (df_proc["WELL"] == str(well)) & (df_proc["Target"] == target.upper())
            ]["CT"]
            # olhar valores brutos para capturar Undetermined textual
            try:
                s_raw = df_filtered[
                    (df_filtered["WELL"] == str(well))
                    & (df_filtered["TARGET NAME"] == target.upper())
                ]["CT"]
            except Exception:
                s_raw = []
        ...
        if swap:
            ...
            try:
                registrar_log(
                    "Análise VR1e2",
                    f"Swap automático aplicado para amostra {samp}: {old_w1}->{well1}, {old_w2}->{well2}",
                    "INFO",
                )
            except Exception:
                pass
    ...
    try:
        ...
        try:
            ...
            for fr in inspect.stack():
                ...
                if "app_state" in locs and hasattr(
                    locs["app_state"], "control_cn_wells"
                ):
        ...
        if caller_app_state is not None and getattr(
            caller_app_state, "control_cn_wells", None
        ):
            control_cn_wells = [w.upper() for w in caller_app_state.control_cn_wells]
        if caller_app_state is not None and getattr(
            caller_app_state, "control_cp_wells", None
        ):
        ...
        amostras_cp = []
        if control_cn_wells:
            amostras_cn = dados_extracao_df[
                dados_extracao_df["Poço"].isin(control_cn_wells)
            ]["Amostra"].tolist()
        else:
            # scan for sample names in mapping that contain 'CN'
            amostras_cn = dados_extracao_df[
                dados_extracao_df["Amostra"]
                .astype(str)
                .str.contains("CN", na=False, case=False)
            ]["Amostra"].tolist()

        if control_cp_wells:
            amostras_cp = dados_extracao_df[
                dados_extracao_df["Poço"].isin(control_cp_wells)
            ]["Amostra"].tolist()
        else:
            amostras_cp = dados_extracao_df[
                dados_extracao_df["Amostra"]
                .astype(str)
                .str.contains("CP", na=False, case=False)
            ]["Amostra"].tolist()
        ...
        # Linha comentada devido a alerta do ruff (F841): variável 'ct_cp_sc2' atribuída mas não utilizada diretamente.
        # ct_cp_sc2 = None
    ...
    for target in TARGET_LIST:
        ...
        df_pivot[coluna_resultado] = df_pivot.apply(
            lambda r: _compute_result(r), axis=1
        )

    df_final = pd.merge(
        dados_extracao_df, df_pivot, left_on="Amostra", right_on="Sample", how="left"
    )
...
class AnalisarPlacaApp(AfterManagerMixin, ctk.CTkToplevel):
    ...
    def _criar_widgets(self):
        ...
        ctk.CTkLabel(
            main_frame,
            text="Processo de Análise",
            font=ctk.CTkFont(size=16, weight="bold"),
        ).pack(pady=20)
        btn = ctk.CTkButton(
            main_frame, text="Iniciar Análise", command=self.iniciar_analise_teste
        )
        btn.pack(pady=10, fill="x")
        # Debug button to show current mapping without running full analysis
        dbg_btn = ctk.CTkButton(
            main_frame, text="Ver Mapeamento (debug)", command=self._debug_show_map
        )
        ...
        ctk.CTkLabel(
            controls_frame,
            text="Override Controles (CN, CP) - Poços (ex: G11,G12):",
            anchor="w",
        ).pack(anchor="w")
        self.cn_entry = ctk.CTkEntry(
            controls_frame, placeholder_text="CN wells (comma-separated)"
        )
        self.cn_entry.pack(fill="x", pady=2)
        self.cp_entry = ctk.CTkEntry(
            controls_frame, placeholder_text="CP wells (comma-separated)"
        )
    ...
    def iniciar_analise_teste(self):
        ...
        if caminho_arquivo:
            ...
            lote_kit = simpledialog.askstring(
                "Input", "Digite o lote de kit:", parent=self
            )
            if lote_kit:
                ...
                if cn_text:
                    self.app_state.control_cn_wells = [
                        s.strip().upper() for s in cn_text.split(",") if s.strip()
                    ]
                if cp_text:
                    self.app_state.control_cp_wells = [
                        s.strip().upper() for s in cp_text.split(",") if s.strip()
                    ]
    ...
    def _debug_show_map(self):
        ...
        try:
            ...
            if df_map is None or df_map.empty:
                messagebox.showinfo(
                    "Mapeamento",
                    "Nenhum mapeamento carregado (execute Mapeamento da Placa primeiro).",
                    parent=self,
                )
                return
...
# ==============================================================================
def iniciar_fluxo_analise(
    master_window, app_state: AppState, lote_kit: str
) -> Optional[pd.DataFrame]:
    try:
        caminho_arquivo = filedialog.askopenfilename(
            title="Selecione o arquivo",
            filetypes=[("Dados", "*.csv *.xlsx *.xls")],
        ...
        try:
            registrar_log(
                "Análise VR1e2",
                f"Mapping (dados_extracao) preview:\n{str(app_state.dados_extracao.head(10))}",
                "DEBUG",
            )
        except Exception:
            pass
        result_df, status = analisar_placa_vr1e2_7500(
            caminho_arquivo, app_state.dados_extracao, app_state.parte_placa
        )
        try:
            registrar_log(
                "Análise VR1e2",
                f"Resultado final preview:\n{str(result_df.head(10))}",
                "DEBUG",
            )
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    class MockAppState:
        def __init__(self):
            self.dados_extracao = pd.DataFrame(
                {"Poço": [], "Amostra": [], "Código": []}
            )
            self.parte_placa = 1
