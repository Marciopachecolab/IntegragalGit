...
class MenuHandler:
    ...
    def abrir_busca_extracao(self):
        ...
        if resultado:
            self.main_window.app_state.dados_extracao, self.main_window.app_state.parte_placa = resultado
            messagebox.showinfo("Sucesso", "Extração carregada com sucesso!", parent=self.main_window)
            self.main_window.update_status(f"{len(self.main_window.app_state.dados_extracao)} amostras carregadas.")
        else:
    ...
    def _obter_detalhes_analise_via_dialogo(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        if self.analysis_service.exames_disponiveis is None:
            messagebox.showerror("Erro de Configuração", "Lista de exames não carregada.", parent=self.main_window)
            return None, None
            
        lista_exames = self.analysis_service.exames_disponiveis['exame'].tolist()
        if not lista_exames:
            messagebox.showwarning("Aviso", "Não há exames configurados para análise.", parent=self.main_window)
            return None, None
        ...
        lote_kit = simpledialog.askstring("Lote do Kit", "Digite o lote do kit utilizado:", parent=self.main_window)
        if not lote_kit:
    ...
    def _executar_servico_analise(self, exame: str, lote: str):
        ...
        """
        try:
            ret = self.analysis_service.executar_analise(self.main_window.app_state, self.main_window, exame, lote)
            resultados_df = None
            ...
            messagebox.showerror("Erro", f"Falha ao executar a análise: {e}", parent=self.main_window)
    
    def realizar_analise(self):
        """Executa análise dos dados carregados"""
        if self.main_window.app_state.dados_extracao is None:
            messagebox.showerror("Erro de Fluxo", "Execute o 'Mapeamento da Placa' primeiro.", parent=self.main_window)
            return
    ...
    def mostrar_resultados_analise(self):
        ...
        try:
            ...
            if df is None or df.empty:
                messagebox.showwarning("Aviso", "Sem resultados para exibir.", parent=self.main_window)
                return
            ...
            TabelaComSelecaoSimulada(
                ...
                agravos, 
                usuario_logado=getattr(self.main_window.app_state, 'usuario_logado', 'Desconhecido')
            )
            
        except Exception as e:
            registrar_log("UI Main", f"Erro ao exibir resultados: {e}", "ERROR")
            messagebox.showerror("Erro", f"Falha ao exibir resultados: {e}", parent=self.main_window)
    ...
    def enviar_para_gal(self):
        ...
        try:
            abrir_janela_envio_gal(self.main_window, self.main_window.app_state.usuario_logado)
        except Exception as e:
    ...
    def gerar_relatorios(self):
        ...
        messagebox.showinfo("Info", "Módulo de relatórios em desenvolvimento.", parent=self.main_window)