...
def _normalizar_resultados_em_memoria(df_raw: pd.DataFrame, config_equip: Dict[str, str]) -> pd.DataFrame:
    col_p = (config_equip.get("coluna_poco") or "").strip()
    col_a = (config_equip.get("coluna_amostra") or "").strip()
    col_t = (config_equip.get("coluna_alvo") or "").strip()
    col_ct = (config_equip.get("coluna_ct") or "").strip()
...
def _ler_e_normalizar_arquivo(contexto: AnaliseContexto) -> pd.DataFrame:
    ...
    if tipo == "csv":
        df_raw = pd.read_csv(caminho, encoding="utf-8")
