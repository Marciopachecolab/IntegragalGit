# ğŸ“‹ PLANO DE IMPLEMENTAÃ‡ÃƒO — FASE 5 (UI de Cadastro/Edição)
...
â”‚ FASE 5 — Plano em 6 Etapas (11-12 horas totais)               â”‚

â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤

â”‚                                                                 â”‚

â”‚ ETAPA 1: Preparação & Design (1-2h)                           â”‚

â”‚   â””â”€ Revisar código + schema + requirements                    â”‚
...
â”‚ ETAPA 4: Formulário Multi-Aba (3h)                            â”‚

â”‚   â””â”€ 6 abas com 13+ campos + widgets                           â”‚

â”‚                                                                 â”‚

â”‚ ETAPA 5: Integração JSON + Registry Reload (2h)               â”‚

â”‚   â””â”€ Save JSON + registry.load() + UI refresh                  â”‚

â”‚                                                                 â”‚

â”‚ ETAPA 6: Validação, Testes & Polimento (1-2h)                â”‚
...
## ğŸ“Š ETAPA 1: Preparação & Design (1-2 horas)
...
- [ ] `services/menu_handler.py` (integração)
...
- [ ] Padrão de _build_tab_* e _load_csv/_save_csv

- [ ] Estrutura de eventos (seleção, novo, salvar)
...
- [ ] Conhecimento do código existente

- [ ] Design da nova aba esboçado
...
class RegistryExamEditor:
    ...
    """Gerencia edição de exames via Registry"""
    ...
    def validate_exam(self, exam_cfg: ExamConfig) -> Tuple[bool, str]:
        ...
        if not exam_cfg.nome_exame or not exam_cfg.nome_exame.strip():
            ...
            errors.append("â€¢ Nome do exame é obrigatório")

        if not exam_cfg.slug or not exam_cfg.slug.strip():

            errors.append("â€¢ Slug é obrigatório")

        if not exam_cfg.tipo_placa_analitica:

            errors.append("â€¢ Tipo de placa é obrigatório")
        ...
        if not exam_cfg.alvos or len(exam_cfg.alvos) == 0:
            ...
            errors.append("â€¢ Alvos não podem estar vazios")

        

        if errors:

            return False, "Erros de validação:\n" + "\n".join(errors)

        return True, "Exame válido"

    

    def reload_registry(self) -> None:

        """Recarrega registry após mudanças"""
...
**Localização:** `services/cadastros_diversos.py`



**Em `_build_ui()`, após criar outras abas:**
...
def _build_tab_exames_registry(self) -> None:
    ...
    """Constrói aba de gerenciamento de exames (registry JSON)"""
...
def _novo_exame_registry(self) -> None:
    ...
    """Abre formulário para novo exame"""
...
def _editar_exame_registry(self) -> None:
    ...
    """Abre formulário para editar exame selecionado"""
...
- [ ] Status label mostrando seleção
...
## ğŸ“� ETAPA 4: Formulário Multi-Aba (3 horas)



### 4.1 Criar Toplevel Window para Formulário



**Método em CadastrosDiversosWindow (~400 linhas):**

```python

def _abrir_formulario_exame(self, slug: Optional[str] = None) -> None:

    """Abre janela com formulário multi-aba para novo/editar exame"""
    ...
    if slug:
        ...
        if not exam_cfg:
            ...
            messagebox.showerror("Erro", f"Exame '{slug}' não encontrado")
...
class ExamFormDialog:
    ...
    """Diálogo para edição de exame com 6 abas"""
...
def _build_tab_basico(self) -> None:
    ...
    # Slug (auto-generated ou editável)
...
- [ ] Método _collect_form_data implementado

- [ ] Método _salvar implementado
...
## ğŸ”„ ETAPA 5: Integração JSON + Registry Reload (2 horas)
...
- [ ] Listbox atualizado após salvar
...
## âœ… ETAPA 6: Validação, Testes & Polimento (1-2 horas)
...
def test_validate_exam_valid():
    ...
    """Testa validação de exame válido"""
...
def test_validate_exam_invalid():
    ...
    """Testa validação de exame inválido"""
...
### 6.2 Testes Manuais (Integração UI)



**Checklist:**

- [ ] Botão "Novo Exame" no menu funciona

- [ ] Formulário abre com 6 abas
...
- [ ] Excluir exame â†’ Confirmação â†’ Arquivo removido â†’ Listbox atualizado
...
- [ ] Permissões de arquivo (try/except)
...
- [ ] Cores indicando campos inválidos

- [ ] Spinner/loader durante save

- [ ] Confirmação ao fechar com dados não salvos
...
- [ ] Testes manuais passam (UI integração)
...
09:00 - 10:30 (1.5h):  ETAPA 1 — Preparação & Design
...
10:30 - 12:30 (2h):    ETAPA 2 — Classe RegistryExamEditor

                        â€¢ Implementar 7 métodos

                        â€¢ Testes de imports

                        

12:30 - 14:00 (1.5h):  ETAPA 3 — UI Aba "Exames (Registry)"

                        â€¢ Listbox + buttons

                        â€¢ Callbacks de seleção
...
09:00 - 12:00 (3h):    ETAPA 4 — Formulário Multi-Aba
...
12:00 - 14:00 (2h):    ETAPA 5 — Integração JSON + Registry
...
09:00 - 10:30 (1.5h):  ETAPA 6 — Testes Unitários
...
10:30 - 12:30 (2h):    ETAPA 6 — Testes Manuais & Polimento
...
12:30 - 14:00 (1h):    Finalização

                        â€¢ Documentação atualizada
...
- [ ] Ler `RELATORIO_FASE5_ANALISE.md` seções 1-4
...
- [ ] Implementar botões: Novo, Editar, Excluir, Recarregar
...
- [ ] Testar listbox atualiza após save
...
3. âœ… "Novo Exame" abre formulário multi-aba

4. âœ… Formulário salva JSON em `config/exams/<slug>.json`

5. âœ… Registry recarrega após save

6. âœ… Exame novo aparece no listbox

7. âœ… "Editar" preenche formulário com dados existentes

8. âœ… "Excluir" remove arquivo JSON

9. âœ… Validação rejeita dados inválidos

10. âœ… Todos testes passam (pytest)

11. âœ… UI polida e funcional

12. âœ… Documentação atualizada (TODO.md, LEITURA_5MIN.md)
...
- [ ] 6 abas formulário implementadas
...
**Próximo Passo:** Iniciar ETAPA 1 — Preparação & Design
...
**Prioridade:** ğŸ”´ **ALTA** — Bloqueante para UI gerenciável
