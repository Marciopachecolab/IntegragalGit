...
def exportar_resultados_gal(df_processado: pd.DataFrame, lote_kit: str,
                            mapeamento_alvo: dict, mapeamento_saida: dict,
                            colunas_modelo: list, colunas_humanas: dict = None) -> None:
    """
    ...
    if df_processado is None or not isinstance(df_processado, pd.DataFrame):
        registrar_log("Erro de ExportaÃ§Ã£o", "DataFrame de resultados invÃ¡lido ou nÃ£o fornecido.", level='ERROR')
        messagebox.showerror("Erro de ExportaÃ§Ã£o", "DataFrame de resultados invÃ¡lido ou nÃ£o fornecido.")
        return
    if mapeamento_alvo is None or not isinstance(mapeamento_alvo, dict):
        registrar_log("Erro de ExportaÃ§Ã£o", "Mapeamento de alvo invÃ¡lido ou nÃ£o fornecido.", level='ERROR')
        messagebox.showerror("Erro de ExportaÃ§Ã£o", "Mapeamento de alvo invÃ¡lido ou nÃ£o fornecido.")
        return
    if mapeamento_saida is None or not isinstance(mapeamento_saida, dict):
        registrar_log("Erro de ExportaÃ§Ã£o", "Mapeamento de saÃ­da invÃ¡lido ou nÃ£o fornecido.", level='ERROR')
        messagebox.showerror("Erro de ExportaÃ§Ã£o", "Mapeamento de saÃ­da invÃ¡lido ou nÃ£o fornecido.")
        return
    if colunas_modelo is None or not isinstance(colunas_modelo, (list, tuple)):
        registrar_log("Erro de ExportaÃ§Ã£o", "Modelo de colunas invÃ¡lido ou nÃ£o fornecido.", level='ERROR')
        messagebox.showerror("Erro de ExportaÃ§Ã£o", "Modelo de colunas invÃ¡lido ou nÃ£o fornecido.")
        return

    # Verificar se hÃ¡ coluna 'Selecionado' e coluna 'Sample' no DataFrame
    if "Selecionado" not in df_processado.columns:
        registrar_log("Erro de ExportaÃ§Ã£o", "Coluna 'Selecionado' nÃ£o encontrada no DataFrame. Nenhuma amostra serÃ¡ exportada.", level='WARNING')
        messagebox.showwarning("Coluna Ausente", "A coluna 'Selecionado' nÃ£o foi encontrada nos resultados. Nenhuma amostra serÃ¡ exportada.")
        return
    if "Sample" not in df_processado.columns:
        registrar_log("Erro de ExportaÃ§Ã£o", "Coluna 'Sample' nÃ£o encontrada no DataFrame.", level='ERROR')
        messagebox.showerror("Erro de ExportaÃ§Ã£o", "NÃ£o foi possÃ­vel encontrar a coluna 'Sample' no DataFrame de resultados.")
        return
            ...
            for _, row in df_processado.iterrows():
                try:
                    ...
                    for internal_col, csv_col in mapeamento_alvo.items():
                        ...
                        if mapped_value == '1':
                            detectable_counts_for_plot[csv_col] = detectable_counts_for_plot.get(csv_col, 0) + 1

                    # Adicionar valores padrÃ£o se as colunas existirem no modelo e nÃ£o foram preenchidas
                    for k, v in {
                        "paciente": "", "exame": "VRSRT", "metodo": "RTTR",
                        "kit": "427", "painel": "1", "resultado": ""
                    }.items():
                    ...
                    registrar_log("Erro na ExportaÃ§Ã£o", f"Amostra {amostra_id} nÃ£o exportada: {e_row}", level='ERROR')
                    continue

    except Exception as e_file:
        registrar_log("Erro na ExportaÃ§Ã£o", f"Falha ao criar log: {e_file}", level='ERROR')
        messagebox.showerror("Erro de ExportaÃ§Ã£o", f"Ocorreu um erro ao criar o log de exportaÃ§Ã£o: {e_file}")
        return

    # Verifica se hÃ¡ alguma amostra vÃ¡lida para exportar
    if not lines_to_export:
        messagebox.showwarning("Nenhuma Amostra", "Nenhuma amostra vÃ¡lida ('Selecionado' com 'âœ“') para exportaÃ§Ã£o.")
        return
    ...
    if output_filepath:
        try:
            ...
            registrar_log("ExportaÃ§Ã£o CSV ConcluÃ­da", f"Arquivo salvo em: {output_filepath}")

            # Construir mensagem de resumo com contagem de detectÃ¡veis por agravo
            msg_to_user = (f"Arquivo salvo em: {output_filepath}\n"
            ...
            for csv_col_name in sorted_targets:
                human_name = colunas_humanas.get(csv_col_name, csv_col_name) if colunas_humanas else csv_col_name
            ...
            messagebox.showerror("Erro de ExportaÃ§Ã£o", f"Ocorreu um erro ao salvar o arquivo CSV: {e_save}")
    else:
        # UsuÃ¡rio cancelou a operaÃ§Ã£o de salvar
        messagebox.showwarning("Cancelado", "ExportaÃ§Ã£o cancelada pelo usuÃ¡rio.")


def _gerar_grafico_detectaveis(detectable_counts: dict, colunas_humanas: dict = None) -> None:
    """
    ...
    if not detectable_counts or all(v == 0 for v in detectable_counts.values()):
        registrar_log("GrÃ¡fico de DetecÃ§Ã£o", "Nenhum alvo detectÃ¡vel para gerar o grÃ¡fico.", level='INFO')
    ...
    plot_labels = [colunas_humanas.get(k, k) if colunas_humanas else k for k in plot_data.keys()]
    ...
    plt.tight_layout()
    plt.show()
    registrar_log("GrÃ¡fico de DetecÃ§Ã£o", "GrÃ¡fico de detectÃ¡veis gerado.", level='INFO')
