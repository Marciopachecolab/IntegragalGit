...
Visualizador de placa (96 poços, porém para uso com exames de 96, 48, 32 ou 24 testes) em CustomTkinter usando dados do
...
# Cores para diferentes tamanhos de grupos (apenas para placas de 48, 32, 24)
...
class WellData:
    ...
    group_position: int = 0  # Posição no grupo (0 = primeiro, 1 = segundo, etc.)
...
class PlateModel:
    ...
    def __init__(self):
        ...
        self.requires_group_frames = False  # Indica se precisa de frames de grupo
        ...
        for idx, row in df.iterrows():
            ...
            for i, poco in enumerate(pocos):
                ...
                wd = WellData(
                    ...
                    group_position=i  # Posição no grupo
    ...
    def _determine_group_frame_requirement(self) -> None:
        ...
        # Verifica se há grupos e se o tipo de placa não é 96

        if self.plate_type in ["48", "32", "24"]:

            self.requires_group_frames = True

            print(f"[INFO] Placa tipo {self.plate_type}: Requer frames de grupo com contorno")

        else:

            self.requires_group_frames = False

            print("[INFO] Placa tipo 96: Sem frames de grupo")
    ...
    def get_group_position(self, well_id: str) -> int:

        """Retorna a posição do poço no grupo."""
    ...
    def update_appearance(self, text: str, color: str, is_selected: bool, 
        ...
        if is_selected:
            ...
            if group_size > 1:
                ...
                # Apenas borda normal para poços em grupo (o contorno será feito externamente)
...
class GroupFrame(ctk.CTkFrame):
    ...
    def _apply_group_border(self):
        ...
        self.configure(border_color=group_color, border_width=10)  # ESPESSURA FIXA 10
...
class PlateView(ctk.CTkFrame):
    ...
    def _init_ui(self):
        ...
        # Primeiro, criar frames de grupo apenas se necessário
        ...
        # Depois, criar os botões individuais
    ...
    def _create_group_frames(self):
        ...
        """Cria frames para agrupar poços quando necessário."""
        ...
        print(f"[INFO] Criando frames de grupo para placa tipo {self.plate_model.plate_type}")

        

        # Para cada grupo, criar um frame

        for group_id, wells in self.plate_model.group_dict.items():

            if len(wells) > 1:  # Apenas grupos com mais de 1 poço

                group_size = len(wells)

                

                # Encontrar a posição na grade para o grupo

                # Vamos posicionar baseado no primeiro poço do grupo

                first_well = self.plate_model.get_well(wells[0])
                ...
                row_idx = ROW_LABELS.index(first_well.row_label) + 1  # +1 para compensar cabeçalho

                col_idx = int(first_well.col_label)  # Coluna começa em 1

                

                # Criar frame do grupo
                ...
                if group_size == 2:
                    ...
                    # Pares ocupam 2 colunas

                    group_frame.grid(row=row_idx, column=col_idx, columnspan=2, 

                                   padx=1, pady=1, sticky="nsew")

                elif group_size == 3:

                    # Trios ocupam 3 colunas

                    group_frame.grid(row=row_idx, column=col_idx, columnspan=3,

                                   padx=1, pady=1, sticky="nsew")

                elif group_size == 4:

                    # Quartetos ocupam 2x2

                    group_frame.grid(row=row_idx, column=col_idx, columnspan=2,

                                   rowspan=2, padx=1, pady=1, sticky="nsew")
        ...
        print(f"[INFO] Criados {len(self.group_frames)} frames de grupo")

    

    def _create_well_buttons(self):

        """Cria botões para todos os poços, posicionando-os dentro dos frames de grupo se necessário."""

        self.well_widgets.clear()
        ...
        for i, row in enumerate(ROW_LABELS, start=1):
            ...
            for j, col in enumerate(COL_LABELS, start=1):
                ...
                # Determinar onde colocar o botão

                parent_frame = self.plate_frame

                grid_kwargs = {"row": i, "column": j, "padx": 1, "pady": 1, "sticky": "nsew"}

                

                # Verificar se o poço pertence a um grupo e se precisamos de frames de grupo

                if well and well.is_grouped and well.group_id and self.plate_model.requires_group_frames:

                    group_frame = self.group_frames.get(well.group_id)

                    if group_frame:

                        parent_frame = group_frame

                        # Posicionar dentro do grupo

                        if well.group_size == 2:

                            grid_kwargs = {"row": 0, "column": well.group_position, 

                                         "padx": 1, "pady": 1, "sticky": "nsew"}

                        elif well.group_size == 3:

                            grid_kwargs = {"row": 0, "column": well.group_position,

                                         "padx": 1, "pady": 1, "sticky": "nsew"}

                        elif well.group_size == 4:

                            # Para quartetos, organizar em grid 2x2

                            row_pos = well.group_position // 2

                            col_pos = well.group_position % 2

                            grid_kwargs = {"row": row_pos, "column": col_pos,
        ...
        print(f"[INFO] Criados {len(self.well_widgets)} botões de poço")
    ...
    def render_plate(self):
        ...
        for well_id, btn in self.well_widgets.items():
            ...
            btn.update_appearance(text, color, is_selected, is_group_highlight, 

                                group_size, group_position)
    ...
    def fill_detail_panel(self, well: WellData):
        ...
        if well.is_grouped and well.paired_wells:
            ...
            group_text = f"{', '.join(well.paired_wells)}"

            

            # Adicionar informação do tipo de grupo
            ...
            if well.group_size == 2:
                ...
                group_text += f" (Grupo de {well.group_size})"
            ...
            self.lbl_group_info.configure(text="Poço individual", text_color="#000000")
