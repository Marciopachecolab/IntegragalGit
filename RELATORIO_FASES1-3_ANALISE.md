...
## 1. FASE 1 ‚Äî Normaliza√ßão dos CSVs
...
#### 1.1 Definir semântica (`tipo_placa` = placa anal√≠tica)



- ‚úÖ **tipo_placa** nos CSVs refere-se ao n√∫mero de po√ßos da placa anal√≠tica (48, 36, 96)

- ‚úÖ **Arquivo:** `banco/placas.csv` ‚Äî contém entradas:

  - `96`: placa de 96 po√ßos (padrão)
...
- RP_min=15, RP_max=35 (padrão global) ‚úì
...
- ‚úÖ **template_exame.json** ‚Äî template genérico criado

- ‚úÖ **schema.json** ‚Äî valida√ßão estrutural definida
...
#### 3.1 Implementa√ßão de `services/exam_registry.py`
...
2. **ExamRegistry class** (91‚Äì270 linhas):
   ...
   - Utilitários: `_read_csv()`, `_read_structured()` (JSON/YAML)



3. **Instância global:**

   - `registry = ExamRegistry()` ‚Äî carregada na inicializa√ßão do m√≥dulo
...
4. **Helper `get_exam_cfg(nome_exame: str) -> ExamConfig`** (270‚Äì296 linhas):
   ...
   - Se não encontrado: **retorna ExamConfig fallback** com valores padrão (detect_max=38.0, RP 15‚Äì35)
...
- VR1, VR2 (hist√≥ricos): tipo_placa=96, RP 15‚Äì35, CT faixas padrão
...
- ‚úì `mapa_alvos` ‚Äî dicionário alias‚Üícanonical (para normaliza√ßão)
...
- ‚úì `normalize_target(name)` ‚Äî mapeia aliases via `mapa_alvos` + normaliza√ßão
...
**‚úÖ Importa√ßão em 3 m√≥dulos cr√≠ticos:**
...
# Retorna: ExamConfig fallback com detect_max=38.0, RP 15‚Äì35 padrão

# Consumidor não quebra
...
| **1** | CSV semântica | ‚úÖ Completo | Alta | tipo_placa = placa anal√≠tica; 48, 36, 96 |
...
| **3** | API Exposta | ‚úÖ Completo | Alta | 13 campos + 2 métodos auxiliares |
...
## 5. INTEGRA√áÉO COM FASE 4 (PREVIEW)
...
- [x] CSV semântica coerente (tipo_placa = anal√≠tica)
...
- [x] Fallback seguro (ExamConfig m√≠nimo se não encontrado)

- [x] Consumidores importam (universal_engine, plate_viewer, history_report)

- [x] Valida√ßão funcional (registry.get() retorna cfg com dados; fallback OK)
...
**Fase 4 ‚Äî Integra√ßão do Registry no C√≥digo:**
...
- Export GAL: filtrar CN/CP/não-numéricos (PATCH 4) ‚úÖ **J√Å COMPLETO**
...
**Conclusão:** O sistema de cadastro de exames (Fases 1‚Äì3) está completamente funcional. O registry h√≠brido está carregando dados de CSVs e JSONs com merge inteligente, e sendo consumido por tr√™s m√≥dulos cr√≠ticos. A arquitetura é robusta com fallbacks apropriados. Fase 4 já foi implementada com 5 patches de integra√ßão (Engine, Map, History, Export, Panel CSV), todos validados com testes.
